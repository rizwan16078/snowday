...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `${SITE}/wind-chill-chart/state/${slug}`;
  ...
  return {
    ...
    alternates: { canonical: `/wind-chill-chart/state/${slug}` },
    openGraph: {
      type: "website",
      url: canonical,
      title,
...
export default async function WindChillStatePage({ params }: Props) {
  ...
  const breadcrumbSchema = breadcrumbListSchema([
    ...
    { name: stateName, path: `/wind-chill-chart/state/${slug}` },
  ]);
